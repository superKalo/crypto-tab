...
window.App.Bitcoin = {
  ...
  initEvents() {
    ...
    App.Settings.get().then(({ period }) => {
      const selectedTab = period
        ? Object.keys(this.PERIODS).indexOf(period)
        : 1;
  ...
  async setPriceChange() {
    ...
    this.$change.innerHTML = ` (<span class="${getVisualClass(
      changePercent
    )}">${getSignedPercentage(changePercent)}</span>
            ${periodLabel})`;
  },
  ...
  setLastUpdated() {
    this.repositories["NOW"].getDataUpToDateStatus().then((info) => {
      ...
      this.$lastUpdated.setAttribute(
        "data-tooltip",
        dayjs(info.lastFetched).calendar()
      );
    });
  },
