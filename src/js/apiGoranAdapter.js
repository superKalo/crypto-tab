...
window.App.apiGoranAdapter = {
  ...
  getBitcoinRatesForAll: function () {
    return this.get(
      "indices/global/history/BTCUSD?period=alltime&?format=json"
    ).then((data) => {
      const nextData = {};
      ...
      return result.map((rec) => ({
        average: parseInt(
          rec.average.reduce((a, b) => a + b) / rec.average.length
        ),
        time: rec.time,
      }));
    });
  ...
  getBitcoinRatesForOneMonth: function () {
    return this.get(
      "indices/global/history/BTCUSD?period=monthly&?format=json"
    ).then((data) => {
      const nextData = {};
  ...
  getBitcoinRatesForOneDay: function () {
    return this.get(
      "indices/global/history/BTCUSD?period=daily&?format=json"
    ).then((data) => {
      const nextData = {};
  ...
  getBitcoinRatesForOneHour: function () {
    return this.get(
      ...
      data.forEach((rec) => {
        ...
        const isWithingAnHour =
          today.valueOf() - date.valueOf() < 60 * 60 * 1000;
